...
export const BasketProduct = ({ product }: { product: IProduct }) => {
  ...
  const updateProductCount = (id: string | number, newCount: number) => {
    const updatedBasket = basket.map((item) =>
      item.productId === id ? { ...item, count: newCount } : item,
    );
    setBasket(updatedBasket);
  };

  const deleteBasketProduct = (id: string | number) => {
    const updatedBasket = basket.filter((item) => item.productId !== id);
    setBasket(updatedBasket);
  };
          ...
          <FaMinus
            onClick={() => {
              if (basketProduct.count > 1) {
                updateProductCount(product.id, basketProduct.count - 1);
              } else {
